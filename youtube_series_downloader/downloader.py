...
class Downloader:
    ...
    def _convert(self):
        ...
        if not config.pretend:
            ...
            completed_process = run([
                ...
                '-filter_complex',
                '[0:v]setpts=({})*PTS[v];[0:a]atempo={}[a]'.format(video_speed, audio_speed),
                '-map', '[v]',
                '-map', '[a]',
                self._tmp_converted
            ]).returncode == 0
    ...
    def _get_out_dir(self) -> str:
        return path.join(config.series_dir, self._channel.collection_dir, self._channel.name, 'Season 01')
    ...
    def _get_out_filepath(self) -> str:
        episode_number = Downloader._db.get_next_episode_number(
            self._channel.name)
